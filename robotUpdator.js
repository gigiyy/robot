...
console.log("we are processsing %s from line %d for %s records.", csvFile, from, count);
if (dryRun) {
...
function doUpdate(unit, robot, newName) {
    ...
    var apiPath = '/odata/Robots(' + id + ')?OrganizationUnitId=' + units.get(unit);
    var apiData = {
        "Id": id,
        "LicenseKey": key,
        "MachineName": machine,
    ...
    orchestrator.put(apiPath, apiData, function (err, data) {
        if (err) {
...
function logMessage(user, machine, newName, msg) {
    fs.appendFile(logFile, util.format("%s::%s::%s::%s::%s\n",
        new Date().toISOString(), user, machine, newName, msg), function (err) {
            if (err) {
...
function checkName(unit, robot, newName) {
...
function getRobotName(unit, user, machine, newName, nextProcess) {
    var apiPath = '/odata/Robots';
    var apiData = {
        'OrganizationUnitId': units.get(unit),
        '$count': true,
...
parser.on('finish', function () {
    ...
    for (i = 0; i < output.length; i++) {
        var robot = output[i];
        findUnitId(robot.unit, function () {
            getRobotName(robot.unit, robot.userName, robot.machine, robot.newName, dryRun ? false : doUpdate);
        });
    }
});
